...
function TripCard({ trip }: { trip: Trip }) {
  ...
  // Generate avatar initials from booking IDs
  const bookingAvatars = trip.bookings.slice(0, 5).map((id, i) => ({
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-blue-200 transition-all duration-200">
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
        ...
        <div className="flex gap-2">
          ...
          {isScheduled || trip.status === "active" ? (
...
function DocumentsPanel({
  ...
  const docList = [
    ...
    },
    {
      ...
      detail: driverUser.vehicleInfo.documents.insuranceCertificate.fileName,
      expiry: "—",
      status: "valid" as const,
      url: driverUser.vehicleInfo.documents.insuranceCertificate.url,
    },
    {
      icon: "🔧",
      name: "Road Worthiness",
      detail: driverUser.vehicleInfo.documents.roadWorthiness.fileName,
      expiry: "—",
      status: "valid" as const,
      url: driverUser.vehicleInfo.documents.roadWorthiness.url,
    },
    {
      icon: "📋",
      name: "Vehicle Registration",
      detail: driverUser.vehicleInfo.documents.registrationCertificate.fileName,
      expiry: "—",
      status: "valid" as const,
      url: driverUser.vehicleInfo.documents.registrationCertificate.url,
    },
  ];
